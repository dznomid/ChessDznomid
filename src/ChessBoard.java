public class ChessBoard {
    ...
    public boolean moveToPosition(int sLine, int sColumn, int eLine, int eColumn) {     // s - start, e - end
        if ((board[sLine][sColumn] != null) && (sLine >= 0) && (sLine <= 7) && (sColumn >= 0) && (sColumn <= 7) ){
            if(nowPlayerColor().equals(board[sLine][sColumn].getColor())){
                if (board[sLine][sColumn].canMoveToPosition(this, sLine, sColumn, eLine, eColumn)){
                    if (board[sLine][sColumn].getSymbol().equals("K") ||  // check position for castling
                            board[sLine][sColumn].getSymbol().equals("R")) {
                        board[sLine][sColumn].check = false;
                    }
                    board[eLine][eColumn] = board[sLine][sColumn];
                    board[sLine][sColumn] = null;
